...
public class Animal2 extends Actor {
	Image imgW1;
	Image imgA1;
	Image imgS1;
	Image imgD1;
	...
	int carD = 0;
	double w = 800;
	ArrayList<End2> inter = new ArrayList<End2>();
	public Animal2(String imageLink) {
	...
	public void act(long now) {
		...
		if (getIntersectingObjects(Log.class).size() >= 1 && !noMove) {
		...
		else if (getIntersectingObjects(End2.class).size() >= 1) {
			inter = (ArrayList<End2>) getIntersectingObjects(End2.class);
			if (getIntersectingObjects(End2.class).get(0).isActivated()) {
				end--;
				points-=50;
			}
			points+=50;
			changeScore = true;
			w=800;
			getIntersectingObjects(End2.class).get(0).setEnd2();
			end++;
			setX(300);
			setY(679.8+movement);
		}
